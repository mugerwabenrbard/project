...
export default function TrainTicketStep() {
  // State for travel date, time, and train ticket upload
  const [travelDate, setTravelDate] = useState('');
  const [travelTime, setTravelTime] = useState('');
  const [trainTicketFile, setTrainTicketFile] = useState(null);

  // Handle file upload for train ticket
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    setTrainTicketFile(file);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h4 className="text-sm font-medium text-gray-800">Train Ticket Details</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-xs font-light text-gray-500 mb-1">Time of Travel</label>
            <input
              type="time"
              value={travelTime}
          ...
          <div>
            <label className="block text-xs font-light text-gray-500 mb-1">Upload Train Ticket (PDF)</label>
            <input
              type="file"
              accept=".pdf"
            ...
            />
            {trainTicketFile && (
              <p className="mt-2 text-xs text-gray-600">Selected: {trainTicketFile.name}</p>
            )}
          </div>
        </div>
      </div>
